...
function buildMarkdown({ batchId, manifest, rows, aggregates, deltas, slowest }) {
  const lines = [
    ...
    `- Installed Chrome (Playwright/CraftDriver): \`${manifest.versions.chrome?.version ?? 'unknown'}\` (${manifest.versions.chrome?.executable ?? 'no executable'})`,
    `- CraftDriver ChromeDriver: \`${manifest.versions.craftdriverBrowser?.chromedriver?.version ?? 'not selected'}\` (${manifest.versions.craftdriverBrowser?.chromedriver?.executable ?? 'no executable'}; major match: ${manifest.versions.craftdriverBrowser?.majorMatch ?? 'n/a'})`,
